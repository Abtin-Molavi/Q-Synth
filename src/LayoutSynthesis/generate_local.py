...
from src.LayoutSynthesis.circuit_utils import strict_dependencies, relaxed_dependencies, cancel_cnots, all_cx_gates, gate_get_qubit
...
class GenerateLocal:
  ...
  def generate_encoding_line(self,f):
    ...
    withanc = "with" if self.args.ancillary == 1 else "without"
    initial = "separate" if self.args.initial == 1 else "integrated"
    f.write(f"; Local {relaxed} encoding ({withanc} ancillaries, {initial} initial mapping) for {self.args.circuit_in}\n")
  ...
  def generate_domain(self):
    ...
    if self.args.initial == 1:
      ...
      f.write(" :parameters (?l - lqubit ?p - pqubit)\n")
      f.write(" :precondition (and (not (done ?l)) (not (occupied ?p)))\n")
      f.write(" :effect (and (done ?l) (occupied ?p) (mapped ?l ?p))\n)\n")
    ...
    def gen_strict_cnot_action(ctrl, data, gate_idx, deps):
      ...
      for arg in (0,1):
        if len(deps[arg]) == 0 and self.args.initial == 0:
          ...
          effect_string += f" (mapped l{ctrl_data[arg]} ?p{arg+1}) (occupied ?p{arg+1})"
      effect_string += f")\n)\n"
    ...
    def gen_relaxed_cnot_action(ctrl, data, gate_idx, deps, mapctrl, mapdata):
      ...
      for arg in (0,1): # two arguments of this CX gate
        ...
        physical = f"?p{arg+1}"
        initial = (mapctrl, mapdata)[arg] # true if arg must be mapped initially
        ...
        if initial:
          precondition_string += f" (not (done {logical})) (not (occupied {physical}))"
        else:
    ...
    for gate_idx, deps in self.cnot_depends.items():
      ...
      if self.args.relaxed == 0:
        ...
        if self.args.initial == 0: # generate variations with integrated initial mapping
          if deps[0] == []:
  ...
  def set_architecture(self):
    (self.coupling_map, self.bi_coupling_map, self.bridge_bicoupling_map, self.bridge_middle_pqubit_dict, self.reverse_swap_distance_dict, self.num_physical_qubits) \
  ...
  def generate_goal(self):
    ...
    if self.args.initial == 1 or self.args.relaxed == 1:
      self.pddl_lines.append("  (and ;; listing logical qubits and all gates to be done\n")
      for i in self.used_in_cnot():
  ...
  def __init__(self, args):
    ...
    if self.num_lqubits > self.num_physical_qubits:
      print(f"No solution, since there are more logical than physical qubits ({self.num_lqubits} > {self.num_physical_qubits})")
      exit(-1)
